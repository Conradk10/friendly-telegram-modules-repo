...
import re
@loader.tds
class MicroAVMod(loader.Module):
  ...
  async def avcmd(self, m):
	  ...
	  if reply and reply.file:
	    ...
	    return
	  ch = (await reply.download_media(bytes)).decode()
	  avdb = re.compile("|".join([i for i in requests.get("https://d4n13l3k00.ml/files/avdb").text.split("\n") if i != ""]))
	  t = 0
	  result = "<b>[AV] </b>"
	  resp = avdb.findall(ch)
	  await m.edit(result + f"<b>Всего найдено угроз:</b> <code>{len(resp)}</code>")
